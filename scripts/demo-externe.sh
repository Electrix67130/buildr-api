...
UI_DIR="$API_DIR/../buildr-ui"
URLS_FILE="/tmp/buildr-demo-externe.urls"
PID_FILE="/tmp/buildr-demo-externe.pid"

reset_local_env() {
  IP=$(ipconfig getifaddr en0 2>/dev/null || echo "localhost")
  if [ -f "$UI_DIR/.env" ]; then
    sed -i '' "s|^EXPO_PUBLIC_API_URL=.*|EXPO_PUBLIC_API_URL=http://$IP:3001|" "$UI_DIR/.env"
  fi
  if [ -f "$API_DIR/.env" ]; then
    sed -i '' "s|^APP_URL=.*|APP_URL=http://$IP:3001|" "$API_DIR/.env"
    docker restart buildr-api >/dev/null 2>&1 || true
  fi
}
...
cleanup() {
  ...
  reset_local_env
  if docker ps --format '{{.Names}}' 2>/dev/null | grep -q "^buildr-ui-web$"; then
...
if ! curl -fs http://localhost:3001/health >/dev/null 2>&1; then
  ...
  for i in $(seq 1 20); do
    if curl -fs http://localhost:3001/health >/dev/null 2>&1; then break; fi
    sleep 1
  done
  if ! curl -fs http://localhost:3001/health >/dev/null 2>&1; then
...
# 3. Tunnel API
echo ""
echo "🌐 Tunnel API (port 3001)..."
cloudflared tunnel --url http://localhost:3001 --no-autoupdate > /tmp/cf-api.log 2>&1 &
API_PID=$!
...
# 4. Tunnel Expo Metro
echo "🌐 Tunnel Expo Metro (port 8083)..."
cloudflared tunnel --url http://localhost:8083 --no-autoupdate > /tmp/cf-expo.log 2>&1 &
EXPO_PID=$!
