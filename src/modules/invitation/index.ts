...
export default fp(
  (fastify, _opts, done) => {
    ...
    // POST /invitations — invite a collaborator (admin or manager only)
    fastify.post('/invitations', { preHandler: [fastify.authenticate] }, async (request, reply) => {
      ...
      if (membership?.role !== 'admin' && membership?.role !== 'manager') {
        return reply.code(403).send({ statusCode: 403, error: 'Forbidden', message: 'Only admins and managers can invite' });
      }

      const data = createInvitationSchema.parse(request.body);

      // Managers can only invite employee or client, not admin/manager
      if (membership.role === 'manager' && (data.role === 'admin' || data.role === 'manager')) {
        return reply.code(403).send({ statusCode: 403, error: 'Forbidden', message: 'Managers can only invite employees or clients' });
      }
